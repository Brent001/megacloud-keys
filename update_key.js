  ...
  try {
    ...
    fs.readFile("output.js", "utf8", async (err, data) => {
      ...
      try {
        ...
        const extra_message = `
                    Decode the following obfuscated script, extract, and retain only the relevant code that directly generates the 64-bit secret key.Remove all irrelevant, unused, or undefined code — keep just the cleaned-up JavaScript that performs the key generation and at the end console.log the key.Do not include comments, explanations, or additional fluff — output code only.
        `;
